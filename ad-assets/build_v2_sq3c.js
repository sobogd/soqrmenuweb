// Square 1:1 (1080x1080). 3 square cards in a row, logo + all text CENTERED.
// Output: ad_v2_sq3c.jpg
...
async function main() {
  ...
  const c1 = await card("c2_menu.png",     "centre");
  const c2 = await card("c2_reserve.webp", "east");
  const c3 = await card("c2_kitchen.jpg",  "centre");
  ...
  const overlay = Buffer.from(
    `<svg width="${W}" height="${H}" xmlns="http://www.w3.org/2000/svg">
      ...
      <text x="${REST_X}" y="${REST_BASE}" font-family="Lato" font-weight="900" font-size="48" fill="#0a0a0a">Rest</text>
  ...
  await base
    ...
    .jpeg({ quality: 92 })
    .toFile("ad_v2_sq3c.jpg");

  console.log(`done ad_v2_sq3c.jpg  (3 cards ${CARD_W}x${CARD_H}, centered)`);
}
