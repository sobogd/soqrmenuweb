...
export default async function ContactsPage({ params }: ContactsPageProps) {
  ...
  return (
    <div className="h-screen relative">
      ...
      {/* Header */}
      <header className="absolute top-0 inset-x-0 h-14 flex justify-center px-5 z-10 bg-black">
        <div className="max-w-[440px] w-full flex items-center relative">
          <Link href={`/m/${slug}`} className="p-2 -ml-2 text-white z-10">
            <ArrowLeft className="h-6 w-6" />
          </Link>
          <h1 className="absolute inset-0 flex items-center justify-center text-lg font-semibold text-white">{t("contacts")}</h1>
        </div>
      </header>

      {/* Action buttons */}
      <nav className="absolute bottom-0 inset-x-0 flex justify-center px-[8%] pb-8 z-10">
        <div className="max-w-[440px] w-full flex flex-col items-end gap-3">
          {restaurant.whatsapp && (
            <a
              href={`https://wa.me/${restaurant.whatsapp.replace(/[^0-9]/g, '')}`}
              target="_blank"
              rel="noopener noreferrer"
              className="h-12 px-4 flex items-center gap-3 rounded-lg shadow-lg bg-white text-black"
            >
