            ...
            <div className="grid gap-4">
              ...
              {GOOGLE_CLIENT_ID && (
                <>
                  ...
                  <div ref={googleHiddenRef} className="absolute w-0 h-0 overflow-hidden" />
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      const el = googleHiddenRef.current?.querySelector('[role="button"]') as HTMLElement
                        || googleHiddenRef.current?.firstElementChild as HTMLElement;
                      el?.click();
                    }}
                    disabled={status === "loading"}
                    className={cn(
                      "flex w-full items-center justify-center gap-2 rounded-[15px] border border-border bg-muted/30 h-10 px-6 text-base lg:h-auto lg:py-2.5 lg:text-lg text-muted-foreground hover:bg-muted/50 transition-colors cursor-pointer disabled:opacity-50 disabled:pointer-events-none",
                      !googleReady && "hidden"
                    )}
                  >
