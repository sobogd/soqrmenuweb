...
export async function generateMetadata({ searchParams }: { searchParams: SearchParamsP }): Promise<Metadata> {
  ...
  return {
    ...
    // canonical всегда на чистый /es — чтобы Google не индексировал ?theme= как дубликаты
    ...
    openGraph: {
      ...
      type: "website",
      images: [{ url: "/og-image.png", width: 1200, height: 630, alt: "IQ Rest carta QR" }],
    },
    twitter: {
...
export default async function LandingPage({ searchParams }: { searchParams: SearchParamsP }) {
  ...
  return (
    <main className="relative">
      ...
      <LandingFooter texts={TEXTS.footer} headerTexts={TEXTS.header} locale="es" />
    </main>
  );
}
