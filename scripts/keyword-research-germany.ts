// Germany keyword research. German = Speisekarte (formal) or Menü.
...
const SEED_KEYWORDS = [
  ...
  "weinkarte digital",
  // Software-intent
  "kassensystem speisekarte",
  "menu app gastronomie",
];
...
async function main() {
  ...
  console.log(`\n${SEED_KEYWORDS.length} seeds, DE geo, German lang...\n`);

  const ideas = await customer.keywordPlanIdeas.generateKeywordIdeas({
    customer_id: process.env.GOOGLE_ADS_CUSTOMER_ID!,
    geo_target_constants: [GERMANY_GEO],
  ...
  fs.writeFileSync(path.join(process.cwd(), "scripts/keyword-research-de.json"), JSON.stringify(rows, null, 2));
  console.log(`\nDumped to scripts/keyword-research-de.json`);
}
